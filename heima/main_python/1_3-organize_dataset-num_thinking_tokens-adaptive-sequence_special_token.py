...
def generate_token_sequence(base_string: str, sequence_length: int) -> str:
    if not base_string or sequence_length <= 0:
        raise ValueError("Base string must be non-empty and sequence length must be greater than 0.")
    
    return ''.join(f"<{base_string}_{i}>" for i in range(1, sequence_length + 1))
...
for idx, whole_sample in enumerate(total_data):
    ...
    for i in range(0, len(whole_sample['conversations']), 2):
        ...
        length_current_summary_tokens = len(model_transform.encode(current_summary)) - 2    # remove start and end token
        length_current_caption_tokens = len(model_transform.encode(current_caption)) - 2
        length_current_reasoning_tokens = len(model_transform.encode(current_reasoning)) - 2

        # length_summary_avg.append(length_current_summary_tokens)
        # length_caption_avg.append(length_current_caption_tokens)
        # length_reasoning_avg.append(length_current_reasoning_tokens)
        # continue

        length_current_num_summary_tokens = max(int(length_current_summary_tokens * compression_ratio_thinking_tokens_summary), 1)
        length_current_num_caption_tokens = max(int(length_current_caption_tokens * compression_ratio_thinking_tokens_caption), 1)
        length_current_num_reasoning_tokens = max(int(length_current_reasoning_tokens * compression_ratio_thinking_tokens_reasoning), 1)
        
        thinking_answer = current_answer.replace(
            current_summary, 
            # "<THINKING_OF_SUMMARY>"*length_current_num_summary_tokens
            generate_token_sequence("THINKING_OF_SUMMARY", length_current_num_summary_tokens)
        ).replace(
            current_caption, 
            # "<THINKING_OF_CAPTION>"*length_current_num_caption_tokens
            generate_token_sequence("THINKING_OF_CAPTION", length_current_num_caption_tokens)
        ).replace(
            current_reasoning, 
            # "<THINKING_OF_REASONING>"*length_current_num_reasoning_tokens
            generate_token_sequence("THINKING_OF_REASONING", length_current_num_reasoning_tokens)
        )
        thinking_sample['conversations'][1]['value'] = thinking_answer

        thinking_answer_summary = current_answer.replace(
            current_summary, 
            # "<THINKING_OF_SUMMARY>"*length_current_num_summary_tokens
            generate_token_sequence("THINKING_OF_SUMMARY", length_current_num_summary_tokens)
        )
        thinking_answer_summary_caption = thinking_answer_summary.replace(
            current_caption, 
            # "<THINKING_OF_CAPTION>"*length_current_num_caption_tokens
            generate_token_sequence("THINKING_OF_CAPTION", length_current_num_caption_tokens)
        )
        thinking_answer_summary_caption_reasoning = thinking_answer_summary_caption.replace(
            current_reasoning, 
            # "<THINKING_OF_REASONING>"*length_current_num_reasoning_tokens
            generate_token_sequence("THINKING_OF_REASONING", length_current_num_reasoning_tokens)
        )
        ...
        count_summary = len(re.findall(r"<THINKING_OF_SUMMARY_\d+>", thinking_answer))
        count_caption = len(re.findall(r"<THINKING_OF_CAPTION_\d+>", thinking_answer))
        count_reasoning = len(re.findall(r"<THINKING_OF_REASONING_\d+>", thinking_answer))
        ...
        both_sample['conversations_compress_summary'] = copy.deepcopy(thinking_sample['conversations'])
        both_sample['conversations_compress_summary'][1]['value'] = thinking_answer_summary
        both_sample['conversations_compress_summary_caption'] = copy.deepcopy(thinking_sample['conversations'])
        both_sample['conversations_compress_summary_caption'][1]['value'] = thinking_answer_summary_caption
        both_sample['conversations_compress_summary_caption_reasoning'] = copy.deepcopy(thinking_sample['conversations'])
        both_sample['conversations_compress_summary_caption_reasoning'][1]['value'] = thinking_answer_summary_caption_reasoning
        both_sample['conversations_compress_caption'] = copy.deepcopy(thinking_sample['conversations'])
        ...
        decode_summary_answer = random.choice(decode_summary_answer_template).format(
            summary="<SUMMARY> "+current_summary+" </SUMMARY>"    # add tag to decoder dataset
        )
        decode_summary_question = random.choice(decode_summary_question_template).format(question=current_question)

        decode_summary_question = decode_summary_question.replace(
            "<THINKING_OF_SUMMARY>", 
            # "<THINKING_OF_SUMMARY>"*length_current_num_summary_tokens
            generate_token_sequence("THINKING_OF_SUMMARY", length_current_num_summary_tokens)
        )
        ...
        decode_caption_answer = random.choice(decode_caption_answer_template).format(
            caption="<CAPTION> "+current_caption+" </CAPTION>"    # add tag to decoder dataset
        )
        decode_caption_question = random.choice(decode_caption_question_template).format(question=current_question)

        decode_caption_question = decode_caption_question.replace(
            "<THINKING_OF_CAPTION>", 
            # "<THINKING_OF_CAPTION>"*length_current_num_caption_tokens
            generate_token_sequence("THINKING_OF_CAPTION", length_current_num_caption_tokens)
        )
        ...
        decode_reasoning_answer = random.choice(decode_reasoning_answer_template).format(
            reasoning="<REASONING> "+current_reasoning+" </REASONING>"    # add tag to decoder dataset
        )
        decode_reasoning_question = random.choice(decode_reasoning_question_template).format(question=current_question)

        decode_reasoning_question = decode_reasoning_question.replace(
...
# print("length_summary_avg:", sum(length_summary_avg)/len(length_summary_avg))
# print("length_caption_avg:", sum(length_caption_avg)/len(length_caption_avg))
# print("length_reasoning_avg:", sum(length_reasoning_avg)/len(length_reasoning_avg))
# print("max length_summary:", max(length_summary_avg))
# print("max length_caption:", max(length_caption_avg))
# print("max length_reasoning:", max(length_reasoning_avg))
# exit()
...
with open("/mnt/localssd/llava-cot-dataset/json_files/data_train-compression_ratio-thinking_token_summary{}_caption{}_reasoning{}-sequence_special_token.json".format(
...
with open("/mnt/localssd/llava-cot-dataset/json_files/data_test-compression_ratio-thinking_token_summary{}_caption{}_reasoning{}-sequence_special_token.json".format(
    compression_ratio_thinking_tokens_summary,
    compression_ratio_thinking_tokens_caption,
    compression_ratio_thinking_tokens_reasoning
), "w") as f:
