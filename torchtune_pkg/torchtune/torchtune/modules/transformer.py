    ...
    def forward(
        ...
        **kwargs: Dict,
        ...
        if encoder_mask is not None:
            # his_TO_DO: remove after PyTorch 2.5 is released
        ...
        # [b, s, d]
        # Norm applied before self-attention
        # his_TO_DO: Add support for sample packing and bring back input_pos
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        # Xuan: add projector initialization
        use_abstract_projection: bool = False,
        projector_input_dim: Optional[int] = None,
        ...
        # Xuan: add for thinking token
        self.last_hidden_state = None

        # Xuan: add abstract projector
        if use_abstract_projection:
            if projector_input_dim is None:
                projector_input_dim = head_dim*num_heads
            abstract_projection = [
                nn.Linear(projector_input_dim, head_dim*num_heads), #, device=tok_embeddings.embedding.weight.device),
                nn.ReLU(inplace=True),
                nn.Linear(head_dim*num_heads, head_dim*num_heads), #, device=tok_embeddings.embedding.weight.device),
                nn.Dropout(0.0),
            ]
            self.abstract_projection = nn.Sequential(*abstract_projection)
        else:
            self.abstract_projection = nn.Identity()
        
        # Xuan: add for UNet-like link between encoder and decoder 
        #   for reducing the hallucination in decoder. (2024 Christmas Try)
        self.hidden_states_at_fusion_interval = []
    ...
    def forward(
        ...
        *,
        ...
        # Xuan: add additional inputs for thinking token replacement
        thinking_token = None,
        thinking_token_mask = None,
        thinking_token_multi_hidden_states = None,
        ...
        # Xuan: replace the thinking token in decode dataset 
        #   with the last hidden state of the main model according to the mask
        #   here, we assume there is the same number of thinking tokens 
        #   and the trues in mask, as we design the dataset in this way
        if thinking_token is not None and thinking_token_mask is not None:
            if type(thinking_token) != list:
                # Xuan: we think about the llava can handle both image tokens and text tokens in the same time, 
                #   thus we may add projector here
                # Xuan: actually and finally, we did not use this 'abstract projection' in our design,
                # thinking_token = self.abstract_projection(thinking_token)

                # thinking_token_mask [batch, num_tokens]
                # thinking_token [batch, num_replace_tokens, dim]
                # h [batch, num_tokens, dim]

                # Xuan: replace here
                flat_mask = thinking_token_mask.view(-1)                     # shape: [batch * num_tokens]
                flat_hidden = h.view(-1, h.shape[-1])                        # shape: [batch * num_tokens, dim]
                flat_thinking_token = thinking_token.view(-1, h.shape[-1])   # shape: [batch * num_replace_tokens, dim]
                
                flat_hidden[flat_mask] = flat_thinking_token

                h = flat_hidden.view(h.shape)                                # shape: [batch, num_tokens, dim]

            elif type(thinking_token) == list and len(thinking_token) == h.shape[0]:
                # Xuan: if there is different number of thinking tokens in different batch,
                #   we need to handle them one by one
                # Xuan: actually and finally, we did not use this 'abstract projection' in our design,
                # for i_batch in range(len(thinking_token)):
                #     thinking_token[i_batch] = self.abstract_projection(thinking_token[i_batch])
                
                # Xuan: replace here
                for i_batch in range(len(thinking_token)):
                    h[i_batch][thinking_token_mask[i_batch]] = thinking_token[i_batch]

            else:
                raise ValueError("There is something wrong with the thinking token replacement")
        ...
        for i, layer in enumerate(self.layers):
            ...
            # Xuan: record the hidden states after the fusion interval
            if hasattr(layer, "fusion_layer"):
                self.hidden_states_at_fusion_interval.append(h)
        
        # shape: [b, s, d]
        h = self.norm(h)

        # Xuan: record the last hidden state for the thinking token inside the main pipeline
        ...
        # his_TO_DO: always output a list to have a consistent output type
